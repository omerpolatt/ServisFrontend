...
export const useRegisterStore = create<AuthState>((set) => ({
  ...
  sendVerificationCode: async (UserMail) => {
    try {
      await axios.post('http://tkk04oksokwwgwswgg84cg4w.5.253.143.162.sslip.io/api/auth/mailcontrol', { UserMail });
  ...
  verifyCode: async (UserMail, verificationCode) => {
    try {
      await axios.post('http://tkk04oksokwwgwswgg84cg4w.5.253.143.162.sslip.io/api/auth/verify', { UserMail, verificationCode });
      set({ isVerified: true });
    } catch (error) {
  ...
  register: async (userData) => {
    try {
      const response = await axios.post('http://tkk04oksokwwgwswgg84cg4w.5.253.143.162.sslip.io/api/auth/register', {
